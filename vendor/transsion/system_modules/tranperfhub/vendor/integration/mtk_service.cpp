...
static void loadTranPerfHub() {
    ALOGI("[TranPerfHub] Loading adapter...");

    void *handle = dlopen("libtranperfhub-adapter.so", RTLD_NOW | RTLD_LOCAL);
    if (!handle) {
        ALOGE("[TranPerfHub] dlopen failed: %s", dlerror());
        return;
    }
    ...
    if (!initFunc) {
        ALOGE("[TranPerfHub] dlsym failed: %s", dlerror());
        dlclose(handle);
        return;
    }

    if (!initFunc()) {
        ALOGE("[TranPerfHub] TranPerfHub_Initialize failed");
        dlclose(handle);
        return;
    }

    ALOGI("[TranPerfHub] Adapter loaded successfully");
}
