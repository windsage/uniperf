...
public final class SmartPlatformMemoryFile {
    ...
    public boolean openAshmem(FileDescriptor fd, int size) throws IOException {
        mFd = SmartPlatformJni.nativeAshmemOpen(fd, size);
        //mPFd = new ParcelFileDescriptor(mFd);
        mSize = size;
        if (mFd != null && mFd.valid()) {
            mAddress = SmartPlatformJni.nativeAshmemMmap(mFd, size, 0);
            if (mAddress != 0) {
                return true;
            } else {
    ...
    public boolean openAshmem(String name, int size) throws IOException {
        if (name != null && size > 0) {
            mFd = SmartPlatformJni.nativeAshmemOpen(name, size);
            mPFd = new ParcelFileDescriptor(mFd);
            mSize = size;
            if (mFd.valid()) {
                mAddress = SmartPlatformJni.nativeAshmemMmap(mFd, size, 0);
    ...
    public int writeBitmap(Bitmap bitmap) {
        ...
        if (!isDeactivated() && mFd.valid()) {
            result = SmartPlatformJni.nativeAshmemWriteBitmap(mFd, mAddress, bitmap);
        } else {
    ...
    public int writeBytes(byte[] buffer, int srcOffset, int destOffset, int count) {
        int result = -1;
        if (!isDeactivated() && mFd.valid()) {
            result = SmartPlatformJni.nativeAshmemWriteBytes(mFd, mAddress, buffer, srcOffset, destOffset, count, false);
        } else {
    ...
    public int readBytes(byte[] buffer, int srcOffset, int destOffset, int count) throws IOException {
        if(!isDeactivated()) {
            return SmartPlatformJni.nativeAshmemReadBytes(mFd, mAddress, buffer, srcOffset, destOffset, count);
        } else {
            Log.e(TAG, "readBytes mAddress invalid = " + mAddress);
            return 0;
        }
    }
    ...
    public void close() {
        if (!isDeactivated()) {
            try {
                SmartPlatformJni.nativeAshmemMunmap(mFd, mAddress, mSize);
                mAddress = 0;
            } catch (IOException e) {
                e.printStackTrace();
            }
        ...
        if (mFd.valid()) {
            ...
            SmartPlatformJni.nativeAshmemClose(mFd);
        }
    }
