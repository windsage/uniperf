...
public class ScrollingFramePrefetcher {
    ...
    private RefreshRatePolicy.VariableRefreshRateListener mVariableRefreshRateListener = null;
    ...
    private ScrollingFramePrefetcher() {
        ...
        if (FEATURE_ENABLE) {
            mScrollStateListener = new ScrollState.ScrollStateListener() {
                ...
                public void onScroll(boolean scrolling) {
                    ...
                    if (Config.isVariableRefreshRateSupported()) {
                        mLowRefreshRateComputeTime = false;
                    }
                }
            };
            ...
            if (Config.isVariableRefreshRateSupported()) {
                mVariableRefreshRateListener = new RefreshRatePolicy.VariableRefreshRateListener() {
                    @Override
                    public void onVariableRefreshRateChanged(int refreshRate) {
                        mVariableRefreshRate = refreshRate;
                        if (mVariableRefreshRate < DISPLAY_RATE_90) {
                            mLowRefreshRateComputeTime = true;
                        }
                        if (LogUtil.DEBUG) {
                            LogUtil.trace(TAG + "#VariableRefreshListener, refreshRate = " + refreshRate
                                + "mLowRefreshRateComputeTime = " + mLowRefreshRateComputeTime);
                        }
                    }
                };
                RefreshRatePolicy.registerVariableRefreshRateListener(mVariableRefreshRateListener);
            }
        }
    }
    ...
    private boolean disableSFP() {
        return !FEATURE_ENABLE || !mIsFling || mForceDisbaleSFP
                || ((ScrollState.getRefreshRate() < DISPLAY_RATE_90) && !mLowRefreshRateComputeTime)
    ...
    public void onFrameEnd(boolean isFrameBegin, FrameScenario scenario) {
        ...
        if (LogUtil.DEBUG) {
            LogUtil.trace(TAG + "#onFrameEnd" + ", is drop frame = " + isDropFrame +
                    ", mIsEstimationFrame = " + mIsEstimationFrame + ", drop reason = " + dropReason);
        }
    ...
    private void doPreAnimation(long frameTimeNano) {
        if (!PRE_ANIM_ENABLE || mChoreographer == null || mForceDisbaleSFP) {
            mIsPreAnim = false;
            return;
        }
        ...
        if (mIsFling != isFling) {
            ...
            if (mIsPreAnimationRunning && !isFling) {
                ...
                if (!mIsFling) {
                    ...
                    mUserTouchDownWhenFling = false;
                    if (PRE_ANIM_ENABLE && mPreAnimationWorkingWhenFling) {
                        Choreographer choreographer;
                        if (mChoreographer != null
                                && (choreographer = mChoreographer.get()) != null) {
    ...
    private void updateFrameIntervalNanos() {
        ...
        if (Config.isVariableRefreshRateSupported()) {
            if (mVariableRefreshRate != -1){
                temp = (long)(1000000000 / mVariableRefreshRate);
            }
            LogUtil.trace(TAG + "#updateFrameIntervalNanos, newTimeNano = " + temp);
        };
    ...
    private void doEstimationFrameHook(long frameTimeNano) {
        ...
        if (mIsAnimDrop || buffercount <= ESTIMATION_BUFFER_COUNT_LIMIT) {
            if (LogUtil.DEBUG) {
