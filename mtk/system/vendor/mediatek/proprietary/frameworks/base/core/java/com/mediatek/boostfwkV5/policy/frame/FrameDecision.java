...
import com.mediatek.boostfwkV5.info.ScrollState.ScrollStateListener;
import com.mediatek.boostfwkV5.policy.frame.HWUIPolicyManager.HWUIPolicyStateListener;
...
public class FrameDecision
        implements HWUIPolicyStateListener, ActivityInfo.ActivityChangeListener,
        ScrollStateListener {
    ...
    private final int FRAME_OP_DEP = 3;
    ...
    private final int DEP_MODE_DEL_SPECIFIC_TASK_BY_NAME = 1;
    private final int DEP_MODE_ADD_SPECIFIC_TASK_BY_NAME = 2;
    private final int DEP_MODE_ADD_SPECIFIC_TASK_BY_TID = 3;
    private final int DEP_MODE_DEL_SPECIFIC_TASK_BY_TID = 4;
    private final int DEP_MODE_CLEAR_SPECIFIC_TASK = 5;
    ...
    //hwui policy cirtical task "tid,tid,..."
    private String mHWUIExtraDepTaskTids = null;
    private int mHWUIExtraDepTaskSize = 0;
    private boolean mClearHWUIExtraDepTask = false;
    private String mDecodeDepTask;
    private int mDecodeDepSize;
    ...
    private FrameDecision() {
        if (Config.getBoostFwkVersion() > Config.BOOST_FWK_VERSION_2) {
            ...
            HWUIPolicyManager.getInstance().registerHWUIPolicyStateListener(this);
            ScrollState.registerScrollStateListener(this);
            getDecodeTasks();
        }
    }

    private void getDecodeTasks() {
        String[] tasks = TasksUtil.DECODE_TASKS;
        mDecodeDepSize = tasks.length;
        mDecodeDepTask = "";
        for (int i = 0; i < mDecodeDepSize; i++) {
            mDecodeDepTask += tasks[i];
            if (i < mDecodeDepSize - 1) {
                mDecodeDepTask += ",";
            }
        }
    }
    ...
    private void onDoFrameBegin(long frameId) {
        ...
        int result = notifySbeFrame(frameId, FRAME_BEGIN, PAGE_TYPE_HWUI);
        int capacity = (result >> 16) & 0xFFFF;
        int enhance = result & 0xFFFF;
        if (capacity <= 100 && capacity > 0) {
            mActivityInfo.recordLastFrameCapacity(capacity);
        }
        mActivityInfo.recordDynamicEnhance(enhance);
        if (LogUtil.DEBUG) {
            LogUtil.traceAndMLogd(TAG, "last capacity is "+capacity+", enhance is "+enhance);
        }
    }
    ...
    private void onDoFrameEnd(long frameId) {
        if (mClearFrameHint ||
            ...
            if (mLastHWUIDoFrameStep < Choreographer.CALLBACK_TRAVERSAL
                    || mFrameDrawnCount == 0) {
                mFrameStatus = 0;
        ...
        if (begin) {
            if (step == Choreographer.CALLBACK_TRAVERSAL) {
                ...
                if (mFrameRequestTraversalCount > 1) {
                    ...
                    ThreadedRenderer.needFrameCompleteHint(
                            ActivityInfo.SBE_MASK_SKIP_FRAME_END_HINT
    ...
    private int notifySbeFrame(long frameId, int frameStatus, int pageType) {
        ...
        if (frameId < 0 && LogUtil.DEBUG) {
            LogUtil.traceAndMLogd(TAG, "mRenderThread "
                    + renderThreadTid + "hint for prefetch frame");
        }
        if (LogUtil.DEBUG) {
        ...
        int mode = DEP_MODE_DEL_SPECIFIC_TASK_BY_NAME;
        String specificTask = "";
        if (frameStatus == FRAME_BEGIN) {
            if (mActivityInfo.isPage(ActivityInfo.PAGE_TYPE_SPECIAL_DESIGN_MAP)) {
                specificTask = "JavaScriptThrea";
                specificTaskSize = 1;
                mode = DEP_MODE_ADD_SPECIFIC_TASK_BY_NAME;
                ...
                //force add decodebitmap task
                specificTask = mDecodeDepTask;
                specificTaskSize = mDecodeDepSize;
                mode = DEP_MODE_ADD_SPECIFIC_TASK_BY_NAME;
            }
        }
        ...
        if (mClearHWUIExtraDepTask) {
            mPowerHalWrap.mtkSbeSetHwuiPolicy(DEP_MODE_CLEAR_SPECIFIC_TASK,
                    "", 0, renderThreadTid, FRAME_OP_DEP, 0, 0);
            mClearHWUIExtraDepTask = false;
        }

        //IOCTRL add ectra critical task to FPSGO
        if (mHWUIExtraDepTaskSize > 0) {
            mPowerHalWrap.mtkSbeSetHwuiPolicy(DEP_MODE_ADD_SPECIFIC_TASK_BY_TID,
                    mHWUIExtraDepTaskTids, mHWUIExtraDepTaskSize,
                    renderThreadTid, FRAME_OP_DEP, 0, 0);
            mHWUIExtraDepTaskSize = 0;
        }

        if (mActivityInfo.isPage(ActivityInfo.PAGE_TYPE_WEBVIEW)) {
            mPowerHalWrap.mtkSbeSetHwuiPolicy(DEP_MODE_ADD_SPECIFIC_TASK_BY_NAME,
                    "AcquireImageThr", 1,
                    renderThreadTid, FRAME_OP_DEP, 0, 0);
        }
        LogUtil.traceEnd();
    ...
    @Override
    public void onHwuiQ2Q(boolean enable) {
        if (mPowerHalWrap == null) {
            mPowerHalWrap = PowerHalWrapper.getInstance();
        }
        ...
        mPowerHalWrap.mtkSbeSetWebviewPolicy("RenderThread", "", 0,
                android.os.Process.myPid(), ScrollPolicy.HWUI_CTRL_MASK,
                enable ? 1 : 0);
    }
    ...
    public void onHwuiPolicyParamChanged(int targetFps, int policy, int[] criticalTask) {
        if (criticalTask != null) {
            int size = criticalTask.length;
            String criticalTids = "";
            for (int i = 0; i < size; i++) {
                criticalTids += String.valueOf(criticalTask[i]);
                if (i != (size - 1)) {
                    criticalTids += ",";
                }
            }
            //already set do not set again
            if (mHWUIExtraDepTaskTids != null
                    && !criticalTids.equals(mHWUIExtraDepTaskTids)) {
                mHWUIExtraDepTaskSize = size;
                mHWUIExtraDepTaskTids = criticalTids;
            }
            mClearHWUIExtraDepTask = false;
            if (LogUtil.DEBUG) {
                LogUtil.traceAndMLogd(TAG, "update HWUI critical="
                        + mHWUIExtraDepTaskTids + " " + mHWUIExtraDepTaskSize);
            }
        } else if (mHWUIExtraDepTaskTids != null) {
            //clear extra dep task
            mClearHWUIExtraDepTask = true;
        }
    }
