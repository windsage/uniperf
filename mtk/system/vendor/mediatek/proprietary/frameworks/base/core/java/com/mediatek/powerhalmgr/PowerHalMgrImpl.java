...
public class PowerHalMgrImpl extends PowerHalMgr {
    ...
    private int perfLockAcqLevel = 202404;
    private boolean perfLockAcqSupported = false;
    ...
    private void init() {
        if (inited == 0) {
            ...
            if(apiLevel <= perfLockAcqLevel) {
                perfLockAcqSupported = true;
                log("perfLockAcq is supported.");
            }else
                log("perfLockAcq is not supported !!!");
        }
    }
...
// @
    public boolean setPriorityByUid(int action, int uid) {
        ...
        try {
            init();
            if (sService != null)
                status = sService.setPriorityByUid(action, uid);
        } catch (RemoteException e) {
    ...
    public boolean setPriorityByLinkinfo(int action, DupLinkInfo  linkinfo){
        ...
        try {
            init();
            if (sService != null)
                status = sService.setPriorityByLinkinfo(action, linkinfo);
        } catch (RemoteException e) {
    ...
    public boolean flushPriorityRules(int type) {
        ...
        try {
            init();
            if (sService != null)
                status = sService.flushPriorityRules(type);
        } catch (RemoteException e) {
    ...
    public boolean configBoosterInfo(BoosterInfo info) {
        ...
        try {
            init();
            if (sService != null) {
                status = sService.configBoosterInfo(info);
            }
        } catch (RemoteException e) {
    ...
    public int perfLockAcquire(int handle, int duration, int[] list) {
        try {
            init();
            if (perfLockAcqSupported && sService != null) {
                int pid = Process.myPid();
                handle = sService.perfLockAcquire(handle, duration, list);
                log("hdl:" + handle + ", pid:" + pid);
            }
        } catch (RemoteException e) {
    ...
    public int perfCusLockHint(int hint, int duration) {
        ...
        try {
            ...
            if (sService != null) {
                int pid = Process.myPid();
                handle = sService.perfCusLockHint(hint, duration);
                log("hdl:" + handle + ", pid:" + pid);
            }
        } catch (RemoteException e) {
    ...
    public void mtkPowerHint(int hint, int data) {
        ...
        try {
            init();
            if (sService != null)
                sService.mtkPowerHint(hint, data);
        } catch (RemoteException e) {
    ...
    public int setSysInfoSync(int type, String data) {
        ...
        try {
            init();
            if (sService != null)
                ret = sService.setSysInfoSync(type, data);
        } catch (RemoteException e) {
            loge("ERR: RemoteException in setPredictInfo:" + e);
        }
        return ret;
