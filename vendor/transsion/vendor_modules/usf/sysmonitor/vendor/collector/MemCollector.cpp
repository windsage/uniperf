...
public:
    ...
    bool init() override {
        ...
        SMLOGI("MemCollector: memFd=%d psiMemFd=%d psiIoFd=%d",
               mMemFd, mMemPsiFd, mIoPsiFd);
        mAvailable = true;
        return true;
    }
    ...
    void sample(const PublishFn& publishFn) override {
        ...
        if (mMemPsiFd >= 0) samplePsi(mMemPsiFd, MetricId::MEM_PRESSURE, "mem",  publishFn, nowNs);
        if (mIoPsiFd  >= 0) samplePsi(mIoPsiFd,  MetricId::IO_PRESSURE,  "io",   publishFn, nowNs);
    }
...
private:
    void sampleMeminfo(const PublishFn& publishFn, int64_t nowNs) {
        if (::lseek(mMemFd, 0, SEEK_SET) < 0) return;
        ...
        while (line && *line) {
            ...
            if      (strncmp(line, "MemFree:",     8)  == 0) memFree   = parseKb(line);
            else if (strncmp(line, "MemAvailable:", 13) == 0) memAvail  = parseKb(line);
            else if (strncmp(line, "Cached:",       7)  == 0) memCached = parseKb(line);
            else if (strncmp(line, "SwapTotal:",   10)  == 0) swapTotal = parseKb(line);
            else if (strncmp(line, "SwapFree:",     9)  == 0) swapFree  = parseKb(line);

            line = next ? next + 1 : nullptr;
        }

        int64_t swapUsed = swapTotal - swapFree;
        SMLOGD("mem: free=%lldkB avail=%lldkB cached=%lldkB swapUsed=%lldkB",
               (long long)memFree, (long long)memAvail,
               (long long)memCached, (long long)swapUsed);
    ...
    void samplePsi(int fd, MetricId id, const char* name,
                   const PublishFn& publishFn, int64_t nowNs) {
        ...
        int tmp = fracPart;
        while (tmp >= 10) { tmp /= 10; fracDiv *= 10; }
    ...
    void closeFds() {
        if (mMemFd    >= 0) { ::close(mMemFd);    mMemFd    = -1; }
        if (mMemPsiFd >= 0) { ::close(mMemPsiFd); mMemPsiFd = -1; }
        if (mIoPsiFd  >= 0) { ::close(mIoPsiFd);  mIoPsiFd  = -1; }
    }
...
ICollector* createMemCollector() { return new MemCollector(); }
