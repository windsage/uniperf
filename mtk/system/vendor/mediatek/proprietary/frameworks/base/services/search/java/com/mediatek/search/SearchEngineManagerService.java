...
public class SearchEngineManagerService extends ISearchEngineManagerService.Stub {
    ...
    public boolean setDefault(SearchEngine engine) {
        String callingPackageName = getCallingPackageName(mContext, Binder.getCallingUid());
        Log.v(TAG,"callingPackageName: "+callingPackageName);
        if (isSystemApp(mContext, callingPackageName)) {
            List<SearchEngine> engines = getAvailables();
            for (SearchEngine eng : engines) {
                if (eng.getName().equals(engine.getName())) {
                    mDefaultSearchEngine = eng;
                    return true;
                }
            }
        }
    ...
    private String getCallingPackageName(Context context,int uid){
