...
static void _Powerd_handleIPC(_tGlobeContext_ * pCtxt, _tTCPCliContext_ * pCliCtxt, int isRead, int isWrite)
{
   ...
   if (isRead && pCliCtxt->TCPState == _TCP_CLIENT_STATE_RUNNING)
   {
      ...
      if (vRet == 0) {
          ...
          else if (vpCmd->CmdID == PS_IPC_COM_TYPE_PERF_LOCK_ACQ)
          {
               struct tPowerData *vpRspMsg = NULL;
               struct tPowerData vPowerData;
               struct tPerfLockData *pPerfLockData;

               vPowerData.msg = POWER_MSG_PERF_LOCK_ACQ;
               vPowerData.pBuf = &(vpCmd->perfLock);

               TWPCDBGP("_Powerd_handleIPC lock_acq, hdl:%d, duration:%d, size:%d\n", vpCmd->perfLock.hdl, vpCmd->perfLock.duration, vpCmd->perfLock.size);

               // call callback to cmd handler with tPowerData
               vHandle = powerd_req(&vPowerData, (void**)&vpRspMsg);

               // send vRet to client
               vpCmd->CmdID = PS_IPC_COM_TYPE_PERF_LOCK_ACQ;

               if (vpRspMsg) {
                   pPerfLockData = (tPerfLockData*)(vpRspMsg->pBuf);
                   if (pPerfLockData != NULL) {
                       TWPCDBGP("_Powerd_handleIPC lock_acq, hdl:%d\n", pPerfLockData->hdl);
                       vpCmd->perfLock.hdl = pPerfLockData->hdl;
                       free(vpRspMsg->pBuf);
                   }
                   free(vpRspMsg);
               }
          ...
          else if (vpCmd->CmdID == PS_IPC_COM_TYPE_PERF_LOCK_REL)
          {
               struct tPowerData *vpRspMsg = NULL;
               struct tPowerData vPowerData;
               struct tPerfLockData vPerfLockData;

               vPerfLockData.hdl = vpCmd->hdlData.hdl;
               vPerfLockData.reserved = vpCmd->hdlData.reserved;

               vPowerData.msg = POWER_MSG_PERF_LOCK_REL;
               vPowerData.pBuf = &vPerfLockData;

               TWPCDBGP("_Powerd_handleIPC lock_rel, hdl:%d\n", vpCmd->hdlData.hdl);
          ...
          else if (vpCmd->CmdID == PS_IPC_COM_TYPE_PERF_CUS_LOCK_HINT)
          {
               struct tPowerData *vpRspMsg = NULL;
               struct tPowerData vPowerData;
               struct tCusLockData *pCusLockData;

               vPowerData.msg = POWER_MSG_CUS_LOCK_HINT;
               vPowerData.pBuf = &(vpCmd->cusLockHint);
               ...
               if (vpRspMsg) {
                   pCusLockData = (tCusLockData*)(vpRspMsg->pBuf);
                   if (pCusLockData != NULL) {
                       vpCmd->cusLockHint.hdl = pCusLockData->hdl;
                       free(vpRspMsg->pBuf);
                   }
                   free(vpRspMsg);
               }
          ...
          else if (vpCmd->CmdID == PS_IPC_COM_TYPE_PERF_TIMER_REQ)
          {
              struct tPowerData *vpRspMsg = NULL;
              struct tPowerData vPowerData;
              //struct tTimerData *pCusLockData;

              vPowerData.msg = POWER_MSG_TIMER_REQ;
              vPowerData.pBuf = &(vpCmd->timerData);
              ...
              if(powerd_cmd_marshall(&pCliCtxt->SendJob, vpCmd) == 0)
                  powerd_ipc_sendcmd(&pCliCtxt->SendJob, pCliCtxt->TCPCliFD);
...
//   int r_anti_clone = drvb_f0();
   ...
   while (1)
   {
      ...
      if (vMaxfd < 0) {
          TWPCDBGP("_selectLoop wrong vMaxfd:%d", vMaxfd);
          break;
      }
...
exit:
   ...
   TWPCDBGP("powerd slave end. vRet:%d", vRet);
