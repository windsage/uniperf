...
import static android.content.pm.PackagePartitions.PARTITION_SYSTEM;
import static android.content.pm.PackagePartitions.PARTITION_VENDOR;
import static android.content.pm.PackagePartitions.PARTITION_PRODUCT;
import static android.content.pm.PackagePartitions.PARTITION_SYSTEM_EXT;
...
import static com.android.server.pm.AppsFilterBase.DEBUG_LOGGING;
import static com.android.server.pm.AppsFilterBase.DEBUG_TRACING;
...
public class PmsExtImpl extends PmsExt {
    ...
    private static String sSysExtRscPath = SystemProperties.get(
            "ro.sys_ext.current_rsc_path", "");
    ...
    // Support skip scan telephony add-on package and ww skip package.
    private static HashSet<String> sSkipScanAppSet = new HashSet<String>();

    private PowerHalWrapper mPowerHalWrapper = null;

    //install powerHAL hint CMD.
    private static final int  PERF_RES_SCHED_CURR_TASK_UCLAMP = 0x01458400;
    private static int mPrePowerHalHandle = -1;
    ...
    @Override
    public void setPerfLockAcquire(boolean enable) {
        if (enable) {
            if (mPrePowerHalHandle == -1) {
                mPrePowerHalHandle = mPowerHalWrapper.perfLockAcquire(0,
                        0,
                        PERF_RES_SCHED_CURR_TASK_UCLAMP,
                        1);
            }
            if (mPrePowerHalHandle <= 0) {
                Slog.i(TAG, "Installation PowerHal perfLockAcquire fail");
            } else {
                Slog.i(TAG, "Installation PowerHal perfLockAcquire success ,"
                        + "perfLockAcquireResult ID is:" + mPrePowerHalHandle);
            }
        } else {
            Slog.i(TAG, "Installation PowerHal perfLockRelease ID is :"+mPrePowerHalHandle);
            mPowerHalWrapper.perfLockRelease(mPrePowerHalHandle);
            mPrePowerHalHandle = -1;
        }
    }

    @Override
    public void scanDirLI(int partitionType, boolean isScanOverlay, int parseFlags, int scanFlags,
            long currentTime, PackageParser2 packageParser, ExecutorService executorService) {
        if ((scanFlags & SCAN_AS_APK_IN_APEX) != 0) {
            return;
        }
        int index = -1;
        boolean isPrivApp = (scanFlags & SCAN_AS_PRIVILEGED) != 0;
        switch (partitionType) {
        case PARTITION_SYSTEM:
            if (isScanOverlay) {
                index = INDEX_RSC_SYSTEM_OVERLAY;
            } else {
                index = isPrivApp ? INDEX_RSC_SYSTEM_PRIV : INDEX_RSC_SYSTEM_APP;
            }
            break;
        case PARTITION_VENDOR:
            if (isScanOverlay) {
                index = INDEX_RSC_VENDOR_OVERLAY;
            } else {
                index = isPrivApp ? INDEX_RSC_VENDOR_PRIV : INDEX_RSC_VENDOR_APP;
            }
            break;
        case PARTITION_PRODUCT:
            if (isScanOverlay) {
                index = INDEX_RSC_PRODUCT_OVERLAY;
            } else {
                index = isPrivApp ? INDEX_RSC_PRODUCT_PRIV : INDEX_RSC_PRODUCT_APP;
            }
            break;
        case PARTITION_SYSTEM_EXT:
            if (isScanOverlay) {
                index = INDEX_RSC_SYSTEM_EXT_OVERLAY;
            } else {
                index = isPrivApp ? INDEX_RSC_SYSTEM_EXT_PRIV : INDEX_RSC_SYSTEM_EXT_APP;
            }
            break;
        default:
            break;
        ...
        if (index > 0) {
            scanDirLI(index, parseFlags, scanFlags, 0, packageParser, executorService);
        }
    }

    @Override
    public void scanDirLI(int index, int parseFlags, int scanFlags,
            long currentTime, PackageParser2 packageParser, ExecutorService executorService) {
        ...
        if (targetFile != null) {
            ...
            mPms.mInitAppsHelper.scanDirTracedLI(targetFile, parseFlags,  scanFlags,
    ...
    public void scanMoreDirLi(int defParseFlags, int defScanFlags,
        ...
        scanDirLI(INDEX_RSC_SYSTEM_PLUGIN, defParseFlags | PackageParser.PARSE_IS_SYSTEM_DIR,
                defScanFlags | SCAN_AS_SYSTEM, 0, packageParser, executorService);

        scanDirLI(INDEX_ROOT_PLUGIN, defParseFlags | PackageParser.PARSE_IS_SYSTEM_DIR,
                defScanFlags | SCAN_AS_SYSTEM, 0, packageParser, executorService);

        scanDirLI(INDEX_RSC_VENDOR_PLUGIN, defParseFlags | PackageParser.PARSE_IS_SYSTEM_DIR,
                defScanFlags | SCAN_AS_SYSTEM | SCAN_AS_VENDOR, 0, packageParser, executorService);

        scanDirLI(INDEX_VENDOR_PLUGIN, defParseFlags | PackageParser.PARSE_IS_SYSTEM_DIR,
                defScanFlags | SCAN_AS_SYSTEM | SCAN_AS_VENDOR, 0, packageParser, executorService);

        scanDirLI(INDEX_VENDOR_OP_APP, defParseFlags | PackageParser.PARSE_IS_SYSTEM_DIR,
                defScanFlags | SCAN_AS_SYSTEM | SCAN_AS_VENDOR, 0, packageParser, executorService);

        scanDirLI(INDEX_CIP_FW, defParseFlags | PackageParser.PARSE_IS_SYSTEM_DIR,
               defScanFlags | SCAN_AS_SYSTEM | SCAN_NO_DEX, 0, packageParser, executorService);
    ...
    @Override
    public void checkMtkResPkg(AndroidPackage pkg)
...
//            }
//            mMediatekApplication = pkg.toAppInfoWithoutState();
//        }
    }

    @Override
    public boolean needSkipScanning(ParsedPackage pkg,
       ...
       if (!mPms.isFirstBoot() && (isRemovableSysApp(pkg.getPackageName())) &&
           (ps == null && updatedPkg == null)) {
           if (mPms.isDeviceUpgrading()) {
               if (!sRemovableSystemAppSetBak.contains(pkg.getPackageName())) {
           ...
           return true;
       } else if (!mPms.isFirstBoot() && pkg != null && ps != null
              && isRemovableSysApp(pkg.getPackageName()) && pkg.getPath().startsWith("/system/")
              && ps.getPathString().startsWith("/data/")) {
    ...
    public void onPackageAdded(String packageName, PackageSetting pkgSetting, int userId) {
        // if the app is  which we care, set its package name to HAL
        if (pkgSetting != null) {
  ...
  @Override
    public int customizeInstallPkgFlags(int installFlags,
            String packageName,
            WatchedArrayMap<String, PackageSetting> settingsPackages, UserHandle user) {
        int ret = installFlags;
        PackageSetting ps = settingsPackages.get(packageName);
        if (ps != null) {
    ...
    public void updatePackageSettings(int userId, String pkgName,
            AndroidPackage newPackage, PackageSetting ps,
            int[] allUsers, String installerPackageName) {
        if (userId == UserHandle.USER_ALL) {
            if (isRemovableSysApp(pkgName) && ps.isSystem()) {
                for (int currentUserId : allUsers) {
                    // OS:add for dualapp by fuyin.liu 20241203 start
                    if (currentUserId == UserHandle.USER_CLONE_PROFILE) {
                        continue;
                    }
                    // OS:add for dualapp by fuyin.liu 20241203 end
    ...
    public ApplicationInfo updateApplicationInfoForRemovable(
        ...
        if (Binder.getCallingPid() == Process.myPid()
                || oldAppInfo == null
                || !isRemovableSysApp(oldAppInfo.packageName)) {
            return oldAppInfo;
        }
        return updateApplicationInfoForRemovable(mPms.snapshotComputer().getNameForUid(Binder
                    ...
                    if (!clearSystemFlag) {
                        try {
                            String[] pkgs = AppGlobals
                                    .getPackageManager()
                                    .getPackagesForUid(UserHandle.getUserId(Binder.getCallingUid())*UserHandle.PER_USER_RANGE+Integer.valueOf(strs[1]));
                            for (String pkg : pkgs) {
    ...
    @Override
    public List<LauncherActivityInfoInternal> updateResolveInfoListForRemovable(
            List<LauncherActivityInfoInternal> apps) throws RemoteException {
        if (apps != null) {
            Iterator<LauncherActivityInfoInternal> it = apps.iterator();
            LauncherActivityInfoInternal info;
            while (it.hasNext()) {
                info = it.next();
                info.getActivityInfo().applicationInfo = updateApplicationInfoForRemovable(
                        AppGlobals.getPackageManager().getNameForUid(
                                Binder.getCallingUid()),
                        info.getActivityInfo().applicationInfo);
            }
        }
        return apps;
    }
    ...
    private void carrierExpressInstall(int defParseFlags, int defScanFlags,
        ...
        if (!isCarrierExpressInstallEnabled) {
            ...
            scanDirLI(INDEX_CUSTOM_APP, defParseFlags | PackageParser.PARSE_IS_SYSTEM_DIR,
                    defScanFlags | SCAN_AS_SYSTEM, currentTime, packageParser, executorService);

            scanDirLI(INDEX_CUSTOM_PLUGIN, defParseFlags | PackageParser.PARSE_IS_SYSTEM_DIR,
                    defScanFlags | SCAN_AS_SYSTEM, currentTime, packageParser, executorService);
        } else {
            // Load custom operator packages
            // carrier express use RSC instead of this to switch operator.
            // the function should be remove at android S.
...
//            try {
//                mPms.scanPackageTracedLI(file, PackageParser.PARSE_IS_SYSTEM_DIR
...
//            }
    ...
    private static void sGetSelfDevelopedAppsPreloadPathsFromFile(
            ArrayMap<String, String> resultMap, File file) {
        FileReader fr = null;
        BufferedReader br = null;
        try {
            if (file.exists()) {
    ...
    private void getAppSetByIntent(HashSet<String> resultSet,
        ...
        if(null == mPms || null == mPms.snapshotComputer()){
            return;
        }
        final List<ResolveInfo> matches = mPms.snapshotComputer().queryIntentActivitiesInternal(
    ...
    private boolean onUpgradeRemovableSystemAppList(HashSet<String> oldSet,
            ...
            WatchedArrayMap<String, PackageSetting> settingsPackages) {
