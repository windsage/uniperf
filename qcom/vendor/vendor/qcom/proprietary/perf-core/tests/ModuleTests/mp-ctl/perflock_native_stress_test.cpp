...
int size_new = (sizeof(perf_locks)/sizeof(perf_locks[0]));
...
static void *perflock_function_hint_renew(void *i) {
    int j;
    ...
    j = 0;
    while(1) {
        int tmp  =  perf_hint_renew(old_handle, 0x00001081, NULL,2000,6,0,NULL);
        if (tmp > 0) {
            old_handle = tmp;
        }
        if (j++ > calls_per_thread) {
            old_handle = 0;
            j = 0;
        }
...
static void perflock_function_perf_event(int params_per_call) {

    if (perf_event == NULL) {
        return ;
    }
    int size = (sizeof(perf_hints)/sizeof(perf_hints[0]));
    printf("calling %s", __func__);
    int k = rand()%size;
    for (int i=0; i<params_per_call;i++) {
        int args[] = {0, duration};
        perf_event(perf_hints[k]+i, NULL, 2,args);
    }
}

static void perflock_function_perf_event_offload(int params_per_call) {

    if (perf_event_offload == NULL) {
        return ;
    }
    int size = (sizeof(perf_hints)/sizeof(perf_hints[0]));
    printf("calling %s\n", __func__);
    ...
    for (int i=0; i<params_per_call;i++) {
        if (k % 2 ==0) {
            perf_event_offload(perf_hints[k]+i, NULL, 0,NULL);
        } else {
            int args[] = {0, 500};
            perf_event_offload(perf_hints[k]+i, NULL, 2,args);
        }
    }
}

static void *perflock_function_hint_acq_rel(void *i) {
    int params_per_call = 2;

    if (perf_hint_acq_rel == NULL) {
        return NULL;
    }
    int size = (sizeof(perf_hints)/sizeof(perf_hints[0]));
    if (i != NULL) {
        int threadno = *((int *) i);
        printf("\ncall from thread %d", threadno);
    }
    srand(time(0));
    sleep(rand() % 6);
    int old_handle = -1;
    ...
    while(1) {
        int k = rand()%size;
        for (int j=0; j<params_per_call;j++) {
            if (perf_hint_acq_rel != NULL) {
                tmp  =  perf_hint_acq_rel(old_handle, perf_hints[k]+j, NULL,duration,0,0,NULL);
            } else {
                QLOGE(LOG_TAG, "perf_hint_acq_rel is NULL");
            }
            old_handle = tmp;
        }
        if ((run_time - getElapsedTime()) < 30) {
...
static void *perflock_function_hint_acq_rel_offload(void *i) {
    int params_per_call = 2;

    if (perf_hint_acq_rel_offload == NULL) {
        return NULL;
    }
    int size = (sizeof(perf_hints)/sizeof(perf_hints[0]));
    if (i != NULL) {
    ...
    while(1) {
        ...
        for (int j=0; j<params_per_call;j++) {
            if (perf_hint_acq_rel_offload != NULL) {
                tmp  =  perf_hint_acq_rel_offload(old_handle, perf_hints[k]+j, NULL,500,0,0,NULL);
            } else {
                QLOGE(LOG_TAG, "perf_hint_acq_rel_offload is NULL");
            }
            if (tmp > 0) {
                old_handle = tmp;
            }
...
static void perflock_function_lock_acq_rel(int params_per_call) {
    int i = 0, j;
    if (perf_lock_acq_rel == NULL) {
        return ;
    }
    int resource_list[MAX_PARAMS_PER_CALL];
    int old_handle = 0;
    ...
    old_handle = perf_lock_acq_rel(old_handle, duration, resource_list, PARAMS_PER_CALL, 0);
    if (duration) {
        struct timespec sleep_time;
        //QLOGE(LOG_TAG, "NRP: Sleep");
        sleep_time.tv_sec = duration / TIME_MSEC_IN_SEC;
...
int main(int argc, char *argv[]) {
    ...
    printf("array size is %d", size_new);
    pthread_attr_t attr;
        ...
        *arg = i;
        ...
        switch(i % testcases) {
            ...
            case 3:
                ...
                break;
            case 4:
                pthread_create(&perflock_thread[i], &attr, perflock_function_hint_renew, (void *)arg);
                break;
            case 5:
                pthread_create(&perflock_thread[i], &attr, perflock_function_hint_acq_rel, (void *)arg);
                break;
            case 6:
                pthread_create(&perflock_thread[i], &attr, perflock_function_hint_acq_rel_offload, (void *)arg);
                break;
            default:
