...
public class PowerHalWifiMonitor {
    ...
    private class MyUEventObserver extends UEventObserver {
        ...
        public void onUEvent(UEventObserver.UEvent event) {
            ...
            int eventCode = 10000000; //default DPP OFF
            if (value != null) {
                Log.i(TAG, "onUEvent, event value:" + value);
                try {
                    eventCode = Integer.parseInt(value);
                    Log.d(TAG, "onUevent, event int code:" + eventCode);
                    ueventCallback(eventCode);
                } catch (Exception e) {
                    Log.e(TAG, "DPP event wrong format:" + e.toString());
                }
            } else {
                Log.e(TAG, "onUEvent, value is NULL");
            }
        }
    }

    /**********************************************************
     * The API was called when wifi firmware report DPP uevent
     * Example
     * - timestamp: 09:42:54.206157, event: 10154206
     *********************************************************/
    private void ueventCallback(int event) {
        ...
        Log.i(TAG, "mDppStarted:" + mDppStarted + ", mWifiConnected:" + mWifiConnected
            + ", busy:" + busy);
        switch (dupEvent) {
