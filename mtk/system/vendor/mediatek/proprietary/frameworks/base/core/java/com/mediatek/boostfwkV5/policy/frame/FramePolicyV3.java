...
public class FramePolicyV3 extends BaseFramePolicy
    ...
    private final int SBE_RESUCE_MODE_UPDATE_RESCUE_STRENGTH = 3;
    ...
    private final int SBE_RESCUE_TYPE_TRAVERSAL_HEAVY = 1 << 13;//12 is using by kernel
    ...
    //rescue check time define
    private static float mFrameIntervalTime = 0;
    private static float mFrameTargetTimeFromHWUI = 0;
    ...
    //hwui policy update time
    private boolean mUpdateHWUIPolicy = false;
    private int mLastHWUIPolicyCMD = -1;
    private int mLastHWUITargetFPS = -1;
    private final int UPDATE_HWUI_POLICY_DELAY = 1;
    //10s hwui scn policy time out
    private final int SCN_HWUI_POLICY_DURATION = (int)(10 * MS_PER_SECONDS);
    ...
    private final int RESCUE_BUFFER_COUNT_THRESHOLD = 3;
    ...
    private final int TRAVERSAL_HEAVY_THRESHOLD = 2;
    ...
    private boolean mTraversalHeayPage = false;
    ...
    private long mCurFrameAnimEndTimeNanos = -1L;
    ...
    private volatile int mDoFrameEndBufferCount = 0;

    private int mRunningRescueFilterCount = Config.isRunningRescueFilter();
    private int mRunningFilterDropCount = 0;
    private long mRunningExecuteTimeNS = 0;
    private long mRunningFilterLimitTimeNS = 0;
    private boolean mDsiableRunningRescueFilter = false;
    ...
    private static class RescueInfoOfScroll {
        ...
        private int mTraversalHeavyCount = 0;
    ...
    private volatile int mCurScrollingTraversalHeavyCount = 0;
    ...
    public static final int MSG_HWUI_POLICY_START = 10;
    public static final int MSG_HWUI_POLICY_END = 11;
    public static final int MSG_RESCUE_PRE_ANIMATION_CHECK = 12;
    public static final int MSG_RESCUE_SECOND_CHECK = 13;
    public static final int MSG_UPDATE_HWUI_POLICY = 14;
    public static final int MSG_EXCUTE_HWUI_POLICY_CMD = 15;
    ...
    protected void handleMessageInternal(Message msg) {
        switch (msg.what) {
            ...
            case MSG_DELAY_STOP_HWUI_HINT:
                dleayStopHwuiHint();
                break;
            case MSG_HWUI_POLICY_START:
                onHWUIPolicyStateChangeInternal(true);
                break;
            case MSG_HWUI_POLICY_END:
                onHWUIPolicyStateChangeInternal(false);
                break;
            ...
            case MSG_RESCUE_SECOND_CHECK:
                secondRescueCheck();
                break;
            case MSG_UPDATE_HWUI_POLICY:
                updateHWUIPolicyInternal();
                break;
            case MSG_EXCUTE_HWUI_POLICY_CMD:
                excuteHWUIPolicyInternal(msg.arg1, msg.arg2 == 1);
                break;
            default:
                break;
        }
    ...
    protected void initCoreServiceInternal() {
        ...
        try {
            ...
            // reset and init core service when next hwui ctl
            mDisplayEventReceiver = null;
            LogUtil.mLoge(TAG, "reset and init core service when next hwui ctl");
            return;
        }
    ...
    private void onDoFrameEnd(long vsyncWakeUpTime, long vsyncDeadlineNanos) {
        ...
        //if this frame no traversal, update buffer count here for running rescue
        if (mDoFrameEndBufferCount == 0) {
            mDoFrameEndBufferCount = mActivityInfo.getBufferCountFromProducer();
        }
        ...
        if (frameTimeNS > 50 * NANOS_PER_MS && !mRescueDoFrame
                && mLastFrameEndFrameId > 0) {
            LogUtil.mLogd(TAG, "do not rescue frame "
                    + mLastFrameEndFrameId +" "+ frameTimeNS);
        }

        if (mRunningExecuteTimeNS != 0) {
            mDsiableRunningRescueFilter = mRunningExecuteTimeNS >= mRunningFilterLimitTimeNS;
            mRunningFilterDropCount++;
        }
        mRunningExecuteTimeNS = 0;
        ...
        if (mCurFrameRescueType != 0) {
            ...
            if ((mCurFrameRescueType & SBE_RESCUE_TYPE_TRAVERSAL_OVER_VSYNC) != 0) {
                ...
                if (traversal2FrameEndTime > mTraversalOvertimeNS) {
                    ...
                    //there is animation and it durtaion less than 0.8ms
                    if (mCurFrameAnimEndTimeNanos > 0
                            && !ScrollState.getFling() //not aosp fling
                            && mDoFrameEndBufferCount <= 2// incase dequeue
                            && (mCurFrameAnimEndTimeNanos - curFrameStartTimeNanos) < 800000) {
                        mCurScrollingTraversalHeavyCount++;
                    }
                }
            }
    ...
    public void doFrameStepHint(boolean isBegin, int step) {
        ...
        if (isBegin) {
            if (step == Choreographer.CALLBACK_ANIMATION
                ...
                mRunningExecuteTimeNS = SystemClock.uptimeNanos();
            } else {
            ...
            if (step == Choreographer.CALLBACK_ANIMATION) {
                ...
                if (!mIsPreAnimation) {
                    mCurFrameAnimEndTimeNanos = SystemClock.uptimeNanos();
                } else {
                    mRunningExecuteTimeNS = SystemClock.uptimeNanos() - mRunningExecuteTimeNS;
                }
    ...
    public void onHwuiPolicyStateChanged(boolean enable) {
        super.onHwuiPolicyStateChanged(enable);
        if (LogUtil.DEBUG) {
            LogUtil.mLogd(TAG,"onHWUIPolicyStateChange enable=" + enable);
        }
        if (mListenFrameHint) {
            mWorkerHandler.removeMessages(MSG_DELAY_STOP_HWUI_HINT);
            updateFrameTask(mFrameHintMask, true);
        }
        mDsiableRunningRescueFilter = mRunningFilterDropCount > 0;
        mRunningFilterDropCount = 0;

        mDoFrameEndBufferCount = 0;

        mWorkerHandler.sendEmptyMessage(enable ? MSG_HWUI_POLICY_START : MSG_HWUI_POLICY_END);
    }

    private void onHWUIPolicyStateChangeInternal(boolean enable) {
        if (enable) {
            if (mNewPageStarted) {
            ...
            if (mCurRescueInfoOfScrolling != null) {
                ...
                mCurRescueInfoOfScrolling.mTraversalOverCount
                        = mCurScrollingTraversalOverCount;
                mCurRescueInfoOfScrolling.mTraversalHeavyCount
                        = mCurScrollingTraversalHeavyCount;
                ...
                mCurScrollingTraversalHeavyCount = 0;
    ...
    @Override
    public void onHwuiPolicyParamChanged(int targetFps, int policy, int[] criticalTask) {
        if (!mCoreServiceReady) {
            return;
        }

        if (policy != mLastHWUIPolicyCMD && mFrameIntervalTime > 0) {
            int enable = 1;
            if (mLastHWUIPolicyCMD > 0 && policy <= 0) {
                //should disable policy!!!
                enable = 0;
            }
            mWorkerHandler.sendMessageAtFrontOfQueue(
                    mWorkerHandler.obtainMessage(MSG_EXCUTE_HWUI_POLICY_CMD, policy, enable));
            mWorkerHandler.sendEmptyMessageDelayed(MSG_UPDATE_HWUI_POLICY,
                    (long)(UPDATE_HWUI_POLICY_DELAY * mFrameIntervalTime));
        }

        if (targetFps != mLastHWUITargetFPS) {
            if (targetFps <= 0) {
                mLastHWUITargetFPS = (int)ScrollState.getRefreshRate();
            } else {
                mLastHWUITargetFPS = targetFps;
            }
            mFrameTargetTimeFromHWUI = MS_PER_SECONDS * 1.0f / mLastHWUITargetFPS;
        }
    }

    private void updateHWUIPolicyInternal() {
        //freature op reset when new page or new HWUI Policy
        Config.resetFeatureOption();
        initialPolicy(null);
        updateLimitVsyncTime(mFrameTargetTimeFromHWUI, mCheckPoint);
    }

    private void excuteHWUIPolicyInternal(int policy, boolean enable) {
        if (mPowerHalService != null) {
            if (LogUtil.DEBUG) {
                LogUtil.traceBeginAndMLogd(TAG, "excuteHWUIPolicyInternal policy="
                        + policy + " enable=" + enable);
            }
            if (enable && policy > 0) {
                mLastHWUIPolicyCMD = policy;
                mPowerHalService.mtkPowerHint(mLastHWUIPolicyCMD, SCN_HWUI_POLICY_DURATION);
            } else if (mLastHWUIPolicyCMD > 0) {
                mPowerHalService.mtkPowerHint(mLastHWUIPolicyCMD, 0);
                mLastHWUIPolicyCMD = -1;
            }
            LogUtil.traceEnd();
        }
    }
    ...
    private void tryUpdateDyRescue() {
        ...
        for (RescueInfoOfScroll scrolling : mRescueInfoOfScrollList) {
            ...
            tHeavyCount += scrolling.mTraversalHeavyCount;
        }
        mEnableOIDynamicRescue = false;
        ...
        if (tHeavyCount >= TRAVERSAL_HEAVY_THRESHOLD
                && mActivityInfo != null && mActivityInfo.getDyRescueEnhance() < 30) {
            mTraversalHeayPage = true;
        }
        if (Config.isTraversalDynamicCheckpoint
                && !mEnableTraversalDynamicRescue
                && (tDropCount > TRAVERSAL_THRESHOLD || mTraversalHeayPage)) {
            mEnableTraversalDynamicRescue = true;
        }

        if (mTraversalHeayPage && mActivityInfo != null) {
            mActivityInfo.markAsTraversalHeavy();
        }
     ...
     * will call from rescuecheckAgain and onVsync (both running in frame policy)
     */
    private void rescueCheck(long vsyncDeadlineNanos) {
        ...
        boolean bufferfilterForRunning = false;
        ...
        if (Config.preRunningRescue
            ...
            if (mDoFrameEndBufferCount >= mRunningRescueFilterCount
                    && !mDsiableRunningRescueFilter) {
                bufferfilterForRunning = true;
            } else {
                mRescueRunning = true;
                mMsgCheckTime = Long.MAX_VALUE;
            }
        ...
        if (LogUtil.DEBUG) {
            LogUtil.trace(mDoFrameEndBufferCount  + " send msg with half:"
                    + halfCheckTime + " one:" + oneVsyncCheckTime);
        }
        ...
        if (mWaitNextVsync || mRescueCheckAgain) {
            ...
            if (mRescueRunning) {
                halfVsyncRescueCheck();
            } else if (!bufferfilterForRunning && !mRescueCheckAgain) {
        ...
        switch(mFrameStep) {
            ...
            case FRAME_STEP_DO_FRAME_TRAVERSAL:
                if (mEnableTraversalDynamicRescue && !mIsTraversalStepEnd) {
                    mRescueDoFrame = true;
                    if (mTraversalHeayPage) {
                        doSBERescue(SBE_RESCUE_TYPE_TRAVERSAL_HEAVY);
                    } else {
    ...
    private void oneVsyncRescueCheck() {
        ...
        if (LogUtil.DEBUG) {
            LogUtil.traceAndMLogd(TAG, "oneVsyncRescueCheck mFrameStep=" + mFrameStep
                    + " mRescueNextFrame="+mRescueNextFrame
                    +" mRescueDoFrame=" + mRescueDoFrame + " frameId=" + mCurFrameId);
        }
    }

    private void checkRecueNextFrame() {
        if (mRunningRescueFilterCount > 0 && !mDsiableRunningRescueFilter) {
            mRescueNextFrame = RESCUE_TO_NEXT_FRAME
                    && mDoFrameEndBufferCount < mRunningRescueFilterCount;
        } else {
            mRescueNextFrame = RESCUE_TO_NEXT_FRAME;
        }
    }
    ...
    private String generateRescueDebugStrFromType(int type) {
        ...
        if ((type & SBE_RESCUE_TYPE_TRAVERSAL_HEAVY) != 0) {
            result += " traversal heavy |";
        }
        return result;
    }
    ...
    private void powerHintForRender(int cmd, String tagMsg) {
        ...
        switch (cmd) {
            case PERF_RES_FPS_FBT_RESCUE_SBE_RESCUE:
                ...
                //filter traversal rescue!!!
                if (mCurFrameId != DEFAULT_FRAME_ID //there is a frame start
                        && mCurFrameRescueMode != SBE_RESUCE_MODE_END
                        && (mCurFrameRescueType & SBE_RESCUE_TYPE_TRAVERSAL_OVER_VSYNC) != 0) {
                    int bufferCount = mDoFrameEndBufferCount;
            ...
            case PERF_RES_UX_SBE_RESCUE_ENHANS:
                mPowerHalWrap.mtkNotifySbeRescue(renderThreadTid,
                            SBE_RESUCE_MODE_UPDATE_RESCUE_STRENGTH, mRescueStrength, 0,0, 0);
                break;
    ...
    private boolean rescueCheckAgainForBufferCountFilter(int bufferCount) {
        mCheckTimeOffset = mHalfVsyncCheckTime;
    ...
    public void onChange(Context c) {
        ...
        if (c != null && c.getClass().getName().contains("ReaderActivity")) {
            mTraversalHeayPage = true;
        }
        ...
        mWorkerHandler.removeMessages(MSG_HWUI_POLICY_START);
        mWorkerHandler.removeMessages(MSG_HWUI_POLICY_END);
    }

    private void resetDynamicRescue() {
        mEnableTraversalDynamicRescue = false;
        mDsiableRunningRescueFilter = false;
        mEnableOIDynamicRescue = false;
        mDisableBufferCountFilterDy = false;
        mDoFrameEndBufferCount = 0;
        mTraversalHeayPage = false;
    }
    ...
    private void updateLimitVsyncTime(float checkPoint) {
        updateLimitVsyncTime(mFrameIntervalTime, checkPoint);
    }

    private void updateLimitVsyncTime(float frameTargetTime, float checkPoint) {
        ...
        mHalfVsyncCheckTime = (float)(frameTargetTime * checkPoint - 0.5);

        float traversalCheckPoint = Config.getTraversalRescueCheckPoint();
        updateOneVsyncCheckTime(traversalCheckPoint);
        mRescueTargetTimeNS = (long)((frameTargetTime - mHalfVsyncCheckTime) * NANOS_PER_MS_F);
        ...
        mTraversalOvertimeNS = (long)(frameTargetTime
                * mTraversalOverTimeThreshold * NANOS_PER_MS_F);
        mFrameIntervalTimeNS = (long)(frameTargetTime * NANOS_PER_MS_F);
        mSecondRescueTimeMS = (long)(frameTargetTime * SECOND_RESCUE_THRESHOLD);
        if (LogUtil.DEBUG) {
        ...
        mRunningFilterLimitTimeNS = mRunningRescueFilterCount * mFrameIntervalTimeNS;
    }
