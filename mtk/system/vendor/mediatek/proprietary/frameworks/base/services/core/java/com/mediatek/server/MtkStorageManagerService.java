...
class MtkStorageManagerService extends StorageManagerService {
    ...
    private ArrayList<WatchedVolumeInfo> findVolumeListNeedFormat() {
        Slog.i(TAG, "findVolumeListNeedFormat");
        ArrayList<WatchedVolumeInfo> tempVolumes = Lists.newArrayList();
        synchronized (mLock) {
    ...
    private void formatPhoneStorageAndExternalSDCard() {
        final ArrayList<WatchedVolumeInfo> tempVolumes = findVolumeListNeedFormat();
        new Thread() {
            public void run() {
                synchronized (FORMAT_LOCK) {
                    int userId = mCurrentUserId;
                    for (int i = 0; i < tempVolumes.size(); i++) {
                        final WatchedVolumeInfo vol = tempVolumes.get(i);
                        // if external sd card is formatted as internal storage
     ...
     * @return
     */
    public boolean isUSBOTG(WatchedVolumeInfo vol) {
