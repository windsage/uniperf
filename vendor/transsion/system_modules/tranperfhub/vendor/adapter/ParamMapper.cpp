...
bool ParamMapper::init(Platform platform) {
    ...
    TLOGI("Initializing ParamMapper for platform: %s", PlatformDetector::getPlatformName(mPlatform));
...
bool ParamMapper::parseXmlDocument(const std::string& xmlPath) {
    ...
    if (xmlPlatform) {
        ...
        TLOGI("XML platform: %s, Current platform: %s", 
              platformStr.c_str(), getPlatformName(mPlatform).c_str());
    }
