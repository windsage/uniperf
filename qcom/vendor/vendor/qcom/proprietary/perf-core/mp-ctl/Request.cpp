...
Request::Request(uint32_t duration, pid_t client_pid, pid_t client_tid, enum client_t client) {
    ...
    mDisplayOffCancelEnabled = false;
...
bool Request::Process(uint32_t nargs, int32_t list[]) {
    ...
    for (i = 0; i < mNumArgs; i++) {
        if (LOCK_NEW_VERSION == state) {
            ...
            // add for do not pend request by chao.xu5 at Aug 18th, 2025 start.
            if (mResources[i].GetOpCode() == MPCTLV3_TRAN_DISPLAY_OFF_CANCEL_CTRL &&
                mResources[i].GetValue() == 1) {
                mDisplayOffCancelEnabled = true;
                QLOGL(LOG_TAG, QLOG_L2, "New opcode display off cancel control enabled, request pid:%" PRId32 " tid:%" PRId32, GetPid(), GetTid());
            }
            // add for do not pend request by chao.xu5 at Aug 18th, 2025 end.
        }
            ...
            if (mResources[i].GetOpCode() == MPCTLV3_TRAN_DISPLAY_OFF_CANCEL_CTRL &&
                mResources[i].GetValue() == 1) {
                mDisplayOffCancelEnabled = true;
                QLOGL(LOG_TAG, QLOG_L2, "Old opcode display off cancel control enabled, request pid:%" PRId32 " tid:%" PRId32, GetPid(), GetTid());
            }
            // add for do not pend request by chao.xu5 at Aug 18th, 2025 end.
        }
        else {
