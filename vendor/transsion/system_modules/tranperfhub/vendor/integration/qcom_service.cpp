...
// QCOM Perf HAL Service - 集成 TranPerfHub
...
#include <dlfcn.h>    // add for TranPerfHub by chao.xu5
...
// add for TranPerfHub by chao.xu5 start.
/**
 * 加载 TranPerfHub 适配器
 *
 * 原理:
 * 1. dlopen 加载 libtranperfhub-adapter.so
 ...
 */
static void loadTranPerfHub() {
    QLOGI(LOG_TAG, "[TranPerfHub] Loading adapter...");

    void *handle = dlopen("libtranperfhub-adapter.so", RTLD_NOW | RTLD_LOCAL);
    if (!handle) {
        QLOGE(LOG_TAG, "[TranPerfHub] dlopen failed: %s", dlerror());
        return;
    }

    // 查找初始化函数
    typedef bool (*InitFunc)(void);
    InitFunc initFunc = reinterpret_cast<InitFunc>(dlsym(handle, "TranPerfHub_Initialize"));
    if (!initFunc) {
        QLOGE(LOG_TAG, "[TranPerfHub] dlsym TranPerfHub_Initialize failed: %s", dlerror());
        dlclose(handle);
        return;
    }
    ...
    if (!initFunc()) {
        QLOGE(LOG_TAG, "[TranPerfHub] TranPerfHub_Initialize failed");
        dlclose(handle);
        return;
    }

    // 不 dlclose,保持 so 加载
    QLOGI(LOG_TAG, "[TranPerfHub] Adapter loaded successfully");
}
// add for TranPerfHub by chao.xu5 end.
...
int main() {
    ...
    // add for TranPerfHub by chao.xu5 start.
    // QCOM 原生服务注册完成后,加载 TranPerfHub 适配器
    loadTranPerfHub();
    // add for TranPerfHub by chao.xu5 end.
