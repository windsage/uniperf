...
#define SYSTEM_ROOT "system_ext/etc"
...
static JNIEnv *load_jni_env()
{
    ...
    handle = dlopen("/system_ext/lib/libcustom_jni.so", RTLD_NOW);
    if (!handle) {
        handle = dlopen("/system_ext/lib64/libcustom_jni.so", RTLD_NOW);
    if (!handle)
        return 0;
    }
    ...
    case '@':
        *parser->buff2.cursor = '\0';
        ...
        if (parser->buff.cursor - parser->buff.base + PROP_VALUE_MAX <= parser->buff.size) {
            for (i = 0;
                    i < sizeof(CustomProperties)/sizeof(CustomProp) && CustomProperties[i].name; i++) {
                if (strcmp(item->prop, CustomProperties[i].name) == 0) {
                    parser->buff.cursor += CustomProperties[i].finder(parser->buff.cursor);
                    break;
                }
            }
            if (i == sizeof(CustomProperties)/sizeof(CustomProp)) {
...
static int get_rsc_file_path(char *buff, int buffSize) {
    ...
    if (strlen(buff) > 0) {
        if (snprintf(&buff[strlen(buff)], buffSize - strlen(buff), "%c%s", '/', CFG_FILE_NAME) > 0)
...
static int get_sys_file_path(char *buff, int buffSize) {
    if (snprintf(buff, buffSize, "%s%c%s", SYSTEM_ROOT, '/', CFG_FILE_NAME) > 0) {
        return 0;
    }
    return -1;
}
