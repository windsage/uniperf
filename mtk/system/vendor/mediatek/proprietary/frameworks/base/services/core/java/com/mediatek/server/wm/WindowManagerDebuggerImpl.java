...
public class WindowManagerDebuggerImpl extends WindowManagerDebugger {
     ...
     */
    @Override
    public int runDebug(PrintWriter pw, String[] args, int opti) {
        int mode = -1;
        String cmd = "help";
        if (opti < args.length) {
            cmd = args[opti];
        ...
        final Field[] allFields = new Field[fields.length + fieldsPolicy.length];
        ...
        while (!setAll && (mode == 0 || opti < args.length)) {
            ...
            for (int i = 0; i < allFields.length; ++i) {
                ...
                if (name != null && (name.startsWith("DEBUG") || name.startsWith("SHOW") ||
                        name.equals("localLOGV"))) {
                    try {
                        if (setAll || name.equals(cmd)) {
                            if (mode != 0) { // enable or disable
                                allFields[i].setAccessible(true);
                                allFields[i].setBoolean(null, mode == 1);
                            ...
                            pw.println(String.format("  %s = %b",
                                name, allFields[i].getBoolean(null)));
                        }
        ...
        // Temporary comment out to fix build errors
        //ProtoLogImpl.getSingleInstance().startProtoLog(pw);
        //ProtoLogImpl.getSingleInstance().enableText(pw, mode == 1);
    ...
    public void debugInterceptKeyBeforeQueueing(String tag, int keycode, boolean interactive,
            ...
            boolean isWakeKey, int result, boolean useHapticFeedback,
            boolean isInjected) {
    ...
    public void debugApplyPostLayoutPolicyLw(String tag, WindowState win,
        ...
        Slog.i(tag, "applyPostLayoutPolicyLw Win " + win +
                ...
                ", win.isGoneForLayout()=" + win.isGoneForLayout() +
                ", attached=" + attached +
                ", imeTarget=" + imeTarget +
    ...
    public void debugViewVisibility(String tag, WindowState win, int viewVisibility,
                int oldVisibility, boolean focusMayChange, int requestedWidth,
                int requestedHeight, ClientWindowFrames outFrames,
        ...
        if (viewVisibility == View.VISIBLE && oldVisibility != View.VISIBLE) {
            Slog.i(tag, "Relayout " + win + ": oldVis=" + oldVisibility
                    ...
                    + " outFrames = " + outFrames
                    + " outSurfaceControl = " + outSurfaceControl);
        }
    }
}
