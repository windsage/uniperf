...
public:
    ...
    virtual int perfGetFeedbackExtn(int req, String16& pkg_name, int tid, int len, int* list) = 0;
    virtual void perfEvent(int eventId, String16& pkg_name, int tid, int len, int* list) = 0;
    virtual int perfHintAcqRel(int handle, int hint, String16& pkg_name, int duration, int hint_type, int tid, int len, int* list) = 0;
    virtual int perfHintRenew(int handle, int hint, String16& pkg_name, int duration, int hint_type, int tid, int len, int* list) = 0;
    virtual double getPerfHalVer() = 0;
    ...
    enum {
        ...
        PERF_GET_FEEDBACK = IBinder::FIRST_CALL_TRANSACTION + 7,
        PERF_EVENT = IBinder::FIRST_CALL_TRANSACTION + 8,
        PERF_HINT_ACQUIRE_RELEASE = IBinder::FIRST_CALL_TRANSACTION + 9,
        PERF_HINT_RENEW = IBinder::FIRST_CALL_TRANSACTION + 10,
        PERF_HAL_VER = IBinder::FIRST_CALL_TRANSACTION + 11,
    };
};
