...
static int (*phwNotifyAppState)(const char *, const char *, int32_t, int32_t, int32_t, int32_t) = NULL;
...
static int (*phwEnableGameMode)(int32_t, int32_t) = NULL;
static int (*phwperfLockAcq)(int32_t, int32_t, int[], int32_t) = NULL;
static int (*phwperfLockRel)(int32_t) = NULL;
static int (*phwperfCusLockHint)(int32_t, int32_t) = NULL;
static int (*phwSetSysInfoAsync)(int32_t, const char *) = NULL;
static int (*phwSbeSetRescue)(int32_t, int32_t, int32_t, int32_t,
        unsigned long long, unsigned long long) = NULL;
static int (*phwSbeSetHwuiPolicy)(int32_t, int32_t, unsigned long long,
        int32_t, const char *, int32_t, int64_t) = NULL;
static int (*phwSbeSetWebviewPolicy)(int32_t, const char *, int64_t, int32_t,const char *, int32_t) = NULL;
static int (*phwSetConsistencyPolicy)(uint32_t, uint32_t, int32_t, int32_t) = NULL;
static int (*phwsbeSetSBB)(int32_t, int32_t, int32_t) = NULL;
...
typedef int (*phw_notify_app_state)(const char *, const char *, int32_t, int32_t, int32_t, int32_t);
...
typedef int (*phw_enable_game_mode)(int32_t, int32_t);
typedef int (*phw_set_sysinfo)(int32_t, const char *);
typedef int (*phw_set_sysinfo_async)(int32_t, const char *);
typedef int (*phw_perf_lock_acq)(int32_t, int32_t, int[], int32_t);
typedef int (*phw_perf_lock_rel)(int32_t);
typedef int (*phw_perf_cus_lock_hint)(int32_t, int32_t);
typedef int (*phw_notify_sbe_rescue)(int32_t, int32_t, int32_t, int32_t,
        unsigned long long, unsigned long long);
typedef int (*phw_sbe_set_hwui_policy)(int32_t, int32_t, unsigned long long,
        int32_t, const char *, int32_t, int64_t);
typedef int (*phw_sbe_set_webview_policy)(int32_t, const char *, int64_t, int32_t, const char *, int32_t);
typedef int (*phw_set_consistency_policy)(uint32_t, uint32_t, int32_t, int32_t);
typedef int (*phw_sbe_set_sbb)(int32_t, int32_t, int32_t);
...
static void load_perf_api(void)
{
    ...
    phwperfCusLockHint = reinterpret_cast<phw_perf_cus_lock_hint>(func);

    if (phwperfCusLockHint == NULL) {
        ALOGE("phwperfCusLockHint error: %s\n", dlerror());
        dlclose(handle);
        return;
    }
...
static int load_SBE_api(void)
{
    ...
    handle = dlopen(PERF_SBE_CTRL_LIB_FULL_NAME, RTLD_NOW);
    if (handle == NULL) {
        ALOGE("load_SBE_api dlopen error: %s", dlerror());
        return -1;
    }

    func = dlsym(handle, "sbeSetRescue");
    phwSbeSetRescue = reinterpret_cast<phw_notify_sbe_rescue>(func);
    if (phwSbeSetRescue == NULL) {
        ALOGE("phwSbeSetRescue error: %s\n", dlerror());
        dlclose(handle);
        return -1;
    }

    func = dlsym(handle, "sbeSetHwuiPolicy");
    phwSbeSetHwuiPolicy = reinterpret_cast<phw_sbe_set_hwui_policy>(func);
    if (phwSbeSetHwuiPolicy == NULL) {
        ALOGE("sbeSetHwuiPolicy error: %s\n", dlerror());
        dlclose(handle);
        return -1;
    }

    func = dlsym(handle, "sbeSetWebviewPolicy");
    phwSbeSetWebviewPolicy = reinterpret_cast<phw_sbe_set_webview_policy>(func);
    if (phwSbeSetWebviewPolicy == NULL) {
        ALOGE("phwSbeSetWebviewPolicy error: %s\n", dlerror());
        dlclose(handle);
        return -1;

    }

    func = dlsym(handle, "fbcConsistencyPolicy");
    phwSetConsistencyPolicy = reinterpret_cast<phw_set_consistency_policy>(func);
    if (phwSetConsistencyPolicy == NULL) {
        ALOGE("phwSetConsistencyPolicy error: %s\n", dlerror());
        dlclose(handle);
        return -1;
    }

    func = dlsym(handle, "sbeSetSBB");
    phwsbeSetSBB = reinterpret_cast<phw_sbe_set_sbb>(func);
    if (phwsbeSetSBB == NULL) {
        ALOGE("phwsbeSetSBB error: %s\n", dlerror());
        dlclose(handle);
        return -1;
    }
...
static void init()
{
    ...
    func = dlsym(handle, "PowerHal_Wrap_setSysInfoAsync");
    phwSetSysInfoAsync = reinterpret_cast<phw_set_sysinfo_async>(func);

    if (phwSetSysInfoAsync == NULL) {
        ALOGE("phwSetSysInfo error: %s", dlerror());
        phwSetSysInfoAsync = NULL;
        dlclose(handle);
        return;
    }

    func = dlsym(handle, "PowerHal_Wrap_EnableGameMode");
    phwEnableGameMode = reinterpret_cast<phw_enable_game_mode>(func);

    if (phwEnableGameMode == NULL) {
        ALOGE("phwEnableGameMode error: %s\n", dlerror());
        dlclose(handle);
        return;
    }
...
static int
com_mediatek_powerhalwrapper_PowerHalWrapper_nativePerfNotifySbeRescue(JNIEnv* /* env */, jobject /* thiz */,
                        jint tid, jint start, jint enhance,
                        jint rescue_type, jlong rescue_target, jlong frame_id)
{
    if (!sbe_inited)
        load_SBE_api();
    // ALOGI("phwSbeSetRescue %d %d %d", tid, start, enhance);
    if (phwSbeSetRescue)
        return phwSbeSetRescue(tid, start, enhance, rescue_type, rescue_target, frame_id);
    return -1;
}

static int
com_mediatek_powerhalwrapper_PowerHalWrapper_nativePerfNotifyFrame(JNIEnv* env, jobject /* thiz */,
                        jstring dep_name, jint dep_mode, jint num,
                        jint tid, jint start, jlong frame_id, jint frame_flags)
{
    if (!sbe_inited)
        load_SBE_api();
    //ALOGI("phwSbeSetHwuiPolicy %d %d", tid, start);
    int ret = -1;
    if (phwSbeSetHwuiPolicy) {
        const char *depName = (dep_name) ? env->GetStringUTFChars(dep_name, 0) : NULL;
        if (depName) {
            ret = phwSbeSetHwuiPolicy(tid, start, frame_id, dep_mode, depName, num, frame_flags);
            env->ReleaseStringUTFChars(dep_name, depName);
        }
    }
    return ret;
}
...
static int
com_mediatek_powerhalwrapper_PowerHalWrapper_nativeSbeSetWebviewPolicy(JNIEnv*  env,
                        jobject /* thiz */, jstring name,jstring specific_name, jint num,
                        jint pid, jint mask, jint start)
{
    if (!sbe_inited)
        load_SBE_api();
    int ret = -1;
    if (phwSbeSetWebviewPolicy) {
        const char *renderName = (name) ? env->GetStringUTFChars(name, 0) : NULL;
        const char *specificName = (specific_name) ? env->GetStringUTFChars(specific_name, 0) : NULL;
        // ALOGI("phwSbeSetWebviewPolicy %d %d %d", pid, mask, start);
        if (renderName && specificName) {
            ret = phwSbeSetWebviewPolicy(pid, renderName, mask, start, specificName, num);
            env->ReleaseStringUTFChars(name, renderName);
            env->ReleaseStringUTFChars(specific_name, specificName);
        }
    }
    return ret;
}
...
static int
com_mediatek_powerhalwrapper_PowerHalWrapper_nativeConsistencyPolicy(JNIEnv* /* env */,
                        jobject /* thiz */, jint enabled, jint pid, jint uclamp_min, jint uclamp_max)
{
    if (!sbe_inited)
        load_SBE_api();

    //ALOGI("MtkConsistencyHint");
    if (phwSetConsistencyPolicy)
        return phwSetConsistencyPolicy(enabled, pid, uclamp_min, uclamp_max);

    return -1;
}

static int
com_mediatek_powerhalwrapper_PowerHalWrapper_nativeSbeSetSBB(JNIEnv* /* env */,
                        jobject /* thiz */, jint pid, jint enabled, jint active_ratio)
{
    if (!sbe_inited)
        load_SBE_api();
    ...
    //ALOGI("phwsbeSetSBB");
    if (phwsbeSetSBB)
        return phwsbeSetSBB(pid, enabled, active_ratio);

    return -1;
}

static int
com_mediatek_powerhalwrapper_PowerHalWrapper_nativeInitSBEAPI(JNIEnv* /* env */, jobject /* thiz */)
{
    if (!sbe_inited)
        load_SBE_api();
    return -1;
...
com_mediatek_powerhalwrapper_PowerHalWrapper_nativeNotifyAppState(JNIEnv *env,
                   jobject /* thiz */, jstring packname, jstring actname, jint pid, jint activityId, jint status, jint uid)
{
    int ret = -1;
    ...
    if (phwNotifyAppState) {
        ...
        ret = phwNotifyAppState(nativePack, nativeAct, pid, activityId, status, uid);
...
static int
com_mediatek_powerhalwrapper_PowerHalWrapper_nativeSetSysInfoAsync(JNIEnv *env,
                   jobject /* thiz */, jstring data, jint type)
{
    int ret = -1;
    ...
    //ALOGI("SetSysInfoAsync");
    if (phwSetSysInfoAsync) {
        const char *nativedata = (data) ? env->GetStringUTFChars(data, 0) : NULL;

        ret = phwSetSysInfoAsync(type, nativedata);
        if (nativedata) {
            env->ReleaseStringUTFChars(data, nativedata);
        }
    }
...
static int
com_mediatek_powerhalwrapper_PowerHalWrapper_nativePerfCusLockHint(JNIEnv* /* env */, jobject /* thiz */,
                         jint hint, jint duration)
{
    if (!inited)
        init();

    //ALOGI("phwperfCusLockHint");
    if (phwperfCusLockHint)
        return phwperfCusLockHint(hint, duration);

    return -1;
}

static int
com_mediatek_powerhalwrapper_PowerHalWrapper_nativeEnableGameMode(JNIEnv* /* env */, jobject /* thiz */,
                        jint enable, jint pid)
{
    if (!inited)
        init();

    if (phwEnableGameMode)
        return phwEnableGameMode(enable, pid);
...
static JNINativeMethod sMethods[] = {
    ...
    {"nativeNotifyAppState",   "(Ljava/lang/String;Ljava/lang/String;IIII)I",
            (int *)com_mediatek_powerhalwrapper_PowerHalWrapper_nativeNotifyAppState},
    {"nativeGameModeEnable",   "(II)I",
            (int *)com_mediatek_powerhalwrapper_PowerHalWrapper_nativeEnableGameMode},
    {"nativeScnReg",   "()I",
    ...
    {"nativeSetSysInfoAsync",   "(Ljava/lang/String;I)I",
            (int *)com_mediatek_powerhalwrapper_PowerHalWrapper_nativeSetSysInfoAsync},
    ...
    {"nativePerfCusLockHint",   "(II)I",
            (int *)com_mediatek_powerhalwrapper_PowerHalWrapper_nativePerfCusLockHint},
    {"nativeNotifySbeRescue",   "(IIIIJJ)I",
            (int *)com_mediatek_powerhalwrapper_PowerHalWrapper_nativePerfNotifySbeRescue},
    {"nativeSbeSetHwuiPolicy",   "(Ljava/lang/String;IIIIJI)I",
            (int *)com_mediatek_powerhalwrapper_PowerHalWrapper_nativePerfNotifyFrame},
    {"nativeSbeSetWebviewPolicy",  "(Ljava/lang/String;Ljava/lang/String;IIII)I",
            (int *)com_mediatek_powerhalwrapper_PowerHalWrapper_nativeSbeSetWebviewPolicy},
    {"nativeConsistencyPolicy",  "(IIII)I",
            (int *)com_mediatek_powerhalwrapper_PowerHalWrapper_nativeConsistencyPolicy},
    {"nativeSbeSetSBB",  "(III)I",
            (int *)com_mediatek_powerhalwrapper_PowerHalWrapper_nativeSbeSetSBB},
    {"nativeInitSBEAPI",  "()I",
            (int *)com_mediatek_powerhalwrapper_PowerHalWrapper_nativeInitSBEAPI},
};
...
extern int register_com_mediatek_scnmodule_ScnModule(JNIEnv* env);
...
extern "C" jint JNI_OnLoad(JavaVM* vm, void* /* reserved */)
{
    ...
    register_com_mediatek_scnmodule_ScnModule(env);
