...
public class StorageManagerEx {
    ...
    /**
     * Returns default path for smartplatformserver.
     * @hide
     * @internal
     */
    @ProductApi
    public static String getDefaultPathForSPMServer() {
        // Modify the default path so that SPMServer can access it
        String path = Environment.getExternalStorageDirectory().getAbsolutePath().replaceFirst("/storage/emulated","/data/media");
        Log.i(TAG, "Default path taken as primary storage for smartplatformserver, path=" + path);
        return path;
    }
    /**
