...
public class LaunchPolicy {
    ...
    private static final int RELEASE_LAUNCH_DURATION = 3000;
    //Release Target Launch hint end CMD.
    private static final int PERF_RES_POWER_END_HINT_HOLD_TIME = 0x03410300;
    ...
    private int[] mPerfLockRsc;

    public LaunchPolicy() {
        mPowerHalService = new Performance();

        if (Config.getLaunchVersion() >= Config.LAUNCH_VERSION_1) {
            mPerfLockRsc = new int[] {PERF_RES_POWER_END_HINT_HOLD_TIME,
                MTKPOWER_HINT_PROCESS_CREATE,
                PERF_RES_POWER_END_HINT_HOLD_TIME,
    ...
    private void perfLockAcquire(int[] resList) {
        if (mPowerHalService != null) {
            mPowerHandle = mPowerHalService.perfLockAcquire(RELEASE_LAUNCH_DURATION, resList);
