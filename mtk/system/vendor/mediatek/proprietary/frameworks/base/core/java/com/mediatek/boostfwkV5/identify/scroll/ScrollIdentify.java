...
package com.mediatek.boostfwkV5.identify.scroll;
...
import com.mediatek.boostfwkV5.info.ActivityInfo.ActivityChangeListener;
...
public class ScrollIdentify extends BaseIdentify{
    ...
    private static int mPageDesign = NO_CHECKED_STATUS;
    private int mScrollType = NO_CHECKED_STATUS;
    private static final int SCROLL_TYPE_GESTURE_SCROLL = 1;
    private static final int SCROLL_TYPE_GESTURE_FLING = 2;
    ...
    private Map<String, Boolean> mScrollerObservers = new HashMap<>();
    ...
    private static class ActivityListener implements ActivityInfo.ActivityChangeListener {
        ...
        @Override
        public void updatePageTypeDone(int pageType) {
        }
    ...
    public boolean dispatchScenario(BasicScenario basicScenario) {
        if (Config.isBoostFwkScrollIdentify() && basicScenario != null) {
            ...
            if (viewContext != null) {
                String contextKey = viewContext.toString();
                // Only check Scrollers that are already being monitored
                if (mScrollerObservers.containsKey(contextKey)) {
                    boolean shouldContinue = ScrollPolicy.getInstance()
                        .checkCustomScroller(contextKey, action);
                    if (!shouldContinue) {
                        // If monitoring should stop (identified as AOSP Scroller), remove it
                        stopOptScrollerObserver(contextKey);
                    }
                }
            }
            ...
            switch(action) {
                case BoostFwkManager.Scroll.INPUT_EVENT:
                    if (event != null) {
                        inputEventCheck(status, event, viewContext);
                    }
                    break;
    ...
    class ScrollGestureListener extends SimpleOnGestureListener {
        ...
        public boolean onScroll(MotionEvent e1, MotionEvent e2,
            ...
            if ((eventDistancX > mMoveDistanceThreshold)
                ...
                if (Config.isBoostFwkLogEnable()) {
                    LogUtil.traceAndMLogd(TAG, "mIsSpecialPageDesign = " + mPageDesign);
                }
                if (mPageDesign == ActivityInfo.NO_CHECKED_STATUS ||
                        mActivityInfo.isPage(ActivityInfo.
                                PAGE_TYPE_SPECIAL_DESIGN_FULLSCREEN_GLTHREAD)) {
                    return false;
                }
                if (Config.isBoostFwkLogEnable()) {
                ...
                mScrollType = SCROLL_TYPE_GESTURE_SCROLL;
                mHaveMoveEvent = true;
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2,
            ...
            if (Math.abs(velocityX) > mActivityInfo.getMinimumFlingVelocity()
                ...
                mScrollType = SCROLL_TYPE_GESTURE_FLING;
    ...
    private void inputEventCheck(int status, MotionEvent motionEvent, Context viewContext) {
        ...
        if (begin) {
            ...
            switch (action) {
                case MotionEvent.ACTION_DOWN:
                    ...
                    // Clear all monitoring states as this is a new scroll
                    clearAllScrollerObserver();
                    ScrollPolicy.getInstance().clearAllScrollerObserver();
                ...
                case MotionEvent.ACTION_CANCEL:
                    ...
                    if (mHaveMoveEvent) {
                        ...
                        if (viewContext != null) {
                            String contextKey = viewContext.toString();
                            startOptScrollerObserver(contextKey);
                        }
                    }
    ...
    private void startOptScrollerObserver(String contextKey) {
        if (contextKey != null) {
            mScrollerObservers.put(contextKey, true);
            // Notify ScrollPolicy to start monitoring this Scroller
            ScrollPolicy.getInstance().startCustomScroller(contextKey);
        }
    }

    // Stop monitoring a specific Scroller
    private void stopOptScrollerObserver(String contextKey) {
        if (contextKey != null) {
            mScrollerObservers.remove(contextKey);
            // Notify ScrollPolicy to stop monitoring this Scroller
            ScrollPolicy.getInstance().stopCustomScroller(contextKey);
        }
    }

    // Stop monitoring all Scrollers
    private void clearAllScrollerObserver() {
        // Clear all observers
        mScrollerObservers.clear();
    }
        ...
        if (!mIsUserTouched || mActivityInfo.isPage(
            ...
            if (Config.isBoostFwkLogEnable()) {
                LogUtil.traceAndMLogd(TAG, "inertialScrollCheck mIsUserTouched="
                        + mIsUserTouched + " mIsSpecialPageDesign="+mPageDesign);
            }
            return;
        }
    ...
    private void inertialScrollHint(boolean enable, String pkgName) {
        ...
        ScrollPolicy.getInstance().releaseTargetFPS(enable, true);
    ...
    private void checkSpecialPageType() {
        mPageDesign = mActivityInfo.getPageType();
    ...
    private void sbeHint(int whichStep, float velocity, String logStr) {
        ...
        if ((whichStep == ScrollPolicy.sFLING_VERTICAL
                ...
                && mPageDesign != ActivityInfo.NO_CHECKED_STATUS) {
            if (velocity >= 0) { // Only cast velocity if it's been provided
                ScrollPolicy.getInstance().scrollHint(
                    whichStep, mPageDesign, (int)velocity);
            } else {
                ScrollPolicy.getInstance().scrollHint(whichStep, mPageDesign);
            }
            return;
        }
