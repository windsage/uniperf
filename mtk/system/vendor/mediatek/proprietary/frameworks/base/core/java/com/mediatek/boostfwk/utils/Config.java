...
public final class Config {
    ...
    private static final String FRAME_RESCUE_STRENGTH = "ro.tr_perf.boostfwk.rescue_strength.config";
    //SPD: add for update rescue strength by song.tang 20240521 end

    // launch version
    public static final String CONFIG_LAUNCH_VERSION = "vendor.boostfwk.launch.version";
    ...
    //SPD: add for update rescue strength by song.tang 20240521 start
    public static final int FRAME_HINT_RESCUE_STRENGTH_MAX = 100;
    public static final int FRAME_HINT_RESCUE_STRENGTH_FLOOR = 30;
    public static final int FRAME_HINT_RESCUE_STRENGTH
            = getIntFromPropWithCheck(FRAME_RESCUE_STRENGTH, 50, 0, 100);
    //SPD: add for update rescue strength by song.tang 20240521 end

    /**--------------------------SBE Version define -----------------**/
    //default version is 1, if systemprop exsit follow prop define;
    private static int mBoostFwkVersion = 0;
    private static int mLaunchVersion = -1;
    ...
    public static final int LAUNCH_VERSION_1 = 1;
    ...
    // only add apps fling better when enable preAnimation, other app(eg.settings) not enable.
    public static final List<String> PKG_PRE_ANIMATION_WHITE_LIST = new ArrayList<String>() {
        {
            add("com.shopee");
    ...
    public static final ArrayList<String> SPECIAL_APP_LIST = new ArrayList<String>() {
        {
            add("com.whatsapp");
            add("com.tencent.mm");
            add("com.facebook.katana");
            add("com.instagram.android");
            add("com.ss.android.lark.transsion");
    ...
    public static final ArrayList<String> SF_SPECIAL_ACTIVITY_LIST = new ArrayList<String>() {
        {
            add("com.tencent.mm.plugin.sns.ui.improve.ImproveSnsTimelineUI");
        }
    };
    ...
    private static void generateDefalutFeatureForDiffVersion() {
        ...
        switch (version) {
            case BOOST_FWK_VERSION_3:
                ...
                //add for enableScrollFloor no matter what the current version is by wei.kang start
                enableScrollFloor = true;
                //add for enableScrollFloor no matter what the current version is by wei.kang end
                mEnableTouchMoveBoost = true;
                SYSTEM_PACKAGE_ARRAY.add("com.android.systemui");
                break;
            case BOOST_FWK_VERSION_4:
                dealyReleaseFPSGO = true;
                mIncreaseBuffer = true;
                mFrameDecisionMode = FRAME_DECISION_MODE_FOR_SCROLLING;
                checkCommonPolicyForALL = true;
    ...
    public static int getScrollCommonHwuiMode() {
        if (mScrollCommonModeHwui <= 0) {
            mScrollCommonModeHwui = getIntFromPropWithCheck(
                ...
                (SCROLLING_COMMON_MODE_120
                | SCROLLING_COMMON_MODE_90),
                0,/**min */
    ...
    public static int getLaunchVersion() {
        if (mLaunchVersion == -1) {
            mLaunchVersion = SystemProperties.getInt(CONFIG_LAUNCH_VERSION, 0);
        }
        return mLaunchVersion;
    }

    // add for pre-animation by shijun.tang start
    public static boolean isInPreAnimationWhiteList(String processName) {
        if (processName == null) {
            return false;
        }
