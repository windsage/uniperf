...
public class PowerHalManagerImpl extends PowerHalManager {
    ...
    public void amsBoostResume(String lastResumedPackageName,
        ...
        //           ", next:" + nextResumedPackageName + "hasATProcess:" + hasAttachedToProcess);
        mPowerHalWrap.amsBoostResume(
                            lastResumedPackageName, nextResumedPackageName, hasAttachedToProcess);
    }

    /*Notify app state*/
    public void amsBoostNotify(int pid, String activityName, String packageName,
                               int activityId, int uid, int state) {
        //Log.e(TAG, "amsBoostNotify pid:" + pid +
        //           ", activity:" + activityName + ", package:" + packageName);
        mPowerHalWrap.amsBoostNotify(pid, activityName, packageName, activityId, uid, state);
    }
    ...
    public void amsNotifyPackageTTID(String packageName, int time, int type) {
        //Log.e(TAG, "amsNotifyPackageTTID:" + packageNamea +
        //           ", time:" + time);
        mPowerHalWrap.amsNotifyPackageTTID(packageName, time, type);
    }

    public void onNotifyAnimationStatus(boolean isOn) {
        //Log.e(TAG, "onNotifyAnimationStatus:" + isOn);
        mPowerHalWrap.onNotifyAnimationStatus(isOn);
    }

    public void onNotifySmartLaunchBoostStatus(boolean isOn) {
        mPowerHalWrap.onNotifySmartLaunchBoostStatus(isOn);
    }

    public void notifyAppUninstall(String packageName) {
        //Log.e(TAG, "notifyAppUninstall :" + packageName);
        mPowerHalWrap.notifyAppUninstall(packageName);
    }
